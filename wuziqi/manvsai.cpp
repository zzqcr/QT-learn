...
void ManVsAI::paintEvent(QPaintEvent *ev)
{
...
void ManVsAI::mousePressEvent(QMouseEvent *ev)
{
...
void ManVsAI::mouseReleaseEvent(QMouseEvent *event)
{
    update();
    if(!isBlack){
        DECISION D = ai.analyse(chessPos,0);
        qDebug()<<D.pos<<D.eval;
        QPoint p = D.pos;
        chessPos[p.x()][p.y()] = 2;

        if(checkWin(p.x(),p.y(),2)){
            QMessageBox::about(nullptr,"获胜","白子获胜");
            init();
        }
        isBlack = true;
...
void ManVsAI::closeEvent(QCloseEvent *ev)
{
    emit windowClosed();
}
